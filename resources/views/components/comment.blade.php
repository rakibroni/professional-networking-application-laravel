...
$answers = $comment->firstFewAnswers();
$isComment = $comment->post()->isNormalPost();
if (!$isComment)  {
    $questionCategories = QuestionCategories::where('question_id', $comment->post()->question()->id)->get();
    $questionComment = $comment->questionComment();
    $isExpertForQuestion = $questionComment->user->isExpertForQuestion($questionCategories);
    $user_like_check = QuestionCommentLike::where('comment_id', $questionComment->id)
        ->where('user_id', auth()->user()->id)
        ->count();
    $total_comment_replies = QuestionCommentReply::where('question_comment_id', '=', $questionComment->id)->count();
    $total_comment_likes = QuestionCommentLike::where('comment_id', $questionComment->id)->count();
}
@endphp
...
<!-- Main Comment -->
<div class="position-relative pb-2" id="comment{{ $comment->uuid }}">

  @if ($comment->answers->count() > 0)
    <div class="comment-main-line" style="height: calc(100%)"></div>
  @endif
  ...
  <div class="row position-relative">
    ...
    @if ($isComment)
      <div class="position-absolute top-0 start-0" style="width: 50px !important">
        <?php Helper::profilePicture('', '40px', $comment->user, 'pointer-on-hover', ''); ?>
      </div>
      <div class="CommentMarginLeftRight " style="background-color: #f2f2f2; border-radius: 5px">
        <div class="py-2 pe-2 ps-2 ps-sm-3">
            ...
            <div style="width: calc(100% - 100px)">
              <div class="d-flex justify-content-between">
                <div class="d-inline-block HideOverflowText"
                  onclick="loadUserProfileCard('{{ $comment->user->username }}')"
                  style="font-weight: 600; font-size: 13px">
                  <span class="underline-on-hover">{{ $comment->user->firstname . ' ' . $comment->user->name }}</span>
                </div>
              ...
              <div class="mb-1 pointer-on-hover HideOverflowText"
                ...
                style="font-size: 12px !important; color: #7F7F7F"><span
                  class="pointer-on-hover">{{ $comment->user->current_position }}{{ $comment->user->currentEmployer(['']) }}</span>
              </div>
            </div>
            ...
            <div class="PostTimeFont d-flex justify-content-end" style="width:100px !important">
              <div>
                <div class="d-inline-block" style="font-size: 12px; font-weight: 400; color: #7F7F7F !important">
                  {{ $comment->created_at->locale('de_DE')->shortRelativeDiffForHumans() }}
                </div>
                <div class="d-flex justify-content-end">
                  ...
                  <ul class="dropdown-menu">
                    <li>
                      <div
                        onclick="openReportModal('{{ $comment->user->fullName() }}', '{{ $comment->user->username }}', 'comment', '{{ $comment->uuid }}', '{{ $comment->post_id }}')"
          ...
          <div class="PostBodyTextFont"
            style="width: 100% !important; white-space: pre-wrap; overflow-wrap: break-word; box-sizing:border-box; vertical-align: top">{{ $comment->text }}</div>
        </div>
      </div>
    @else
      <div class="position-absolute top-0 start-0" style="width: 50px !important">
      ...
      <div class="CommentMarginLeftRight comment-question-border">
            ...
            <div style="width: calc(100% - 100px)">
              <div class="d-flex justify-content-between">
                <div class="d-inline-block HideOverflowText"
                  ...
                  style="font-weight: 600; font-size: 13px">
                  <span
                    class="underline-on-hover">{{ $comment->user->firstname . ' ' . $comment->user->name }}</span>
                </div>
              </div>
...
<div class="d-flex justify-content-start mt-1 mb-2 unselectable" style="margin-left: 40px">
  @if (!$isComment)
      ...
      <div>
        <a role="button" class="icon col-auto">
          @if ($user_like_check == 0)
            <img width="25" height="25" onclick="addCommentLike({{ $questionComment->id }})"
              id="comment_like_{{ $questionComment->id }}" src="{{ asset('images/helpful-icon.svg') }}"
              class="pointer-on-hover pb-1" alt="" />
          @else
            <img width="25" height="25" onclick="removeCommentLike({{ $questionComment->id }})"
              id="comment_like_{{ $questionComment->id }}" src="{{ asset('images/active-helpful-icon.svg') }}"
              class="pointer-on-hover  pb-1" alt="" />
          @endif
          {{-- <span role="svg">
          ...
          <span id="like_count_{{ $questionComment->id }}"> {{ $total_comment_likes }}</span>
        </a>
      </div>
    </div>

  @endif
  {{-- <div class="PostLikeFont ps-2 ps-sm-3 pe-3"><img class="align-self-center mb-1 pe-1" onclick="" id="" src="{{ asset('images/Like.svg') }}" alt=""> 5</div> --}}
  <div class="PostLikeFont ps-2 pe-3"><img class="align-self-center mb-1" onclick="" id=""
      ...
      id="comment_answers_count{{ $comment->uuid }}">{{ $comment->totalAnswersCount() }}</span> </div>
  <div class="CommentVerticalLine" style="color: white">.</div>
  <div data-bs-toggle="collapse" data-bs-target="#comment_collapse{{ $comment->uuid }}"
    class="ps-3 PostLikeFont underline-on-hover align-self-center" style="font-size: 14px !important">Antworten
  </div>
</div>

<div class="row " style="margin-left: 45px !important">
  {{-- <div class="d-none d-flex justify-content-start mt-1 mb-2">
      <div class="d-none PostLikeFont ps-2 ps-sm-3 pe-3"><img class="d-none align-self-center mb-1 pe-1" onclick=""
          id="" src="{{ asset('images/Like.svg') }}" alt=""></div>
      <div class="PostLikeFont ps-2 pe-3"><img class="align-self-center mb-1 pe-1" onclick="" id=""
          src="{{ asset('images/Reply.svg') }}" alt=""></div>
      <div class="CommentVerticalLine d-none"></div>
      <div class="ps-3 PostLikeFont underline-on-hover align-self-center d-none" style="font-size: 14px !important">
        Antworten</div>
    </div> --}}

  <div class="collapse pb-3" id="comment_collapse{{ $comment->uuid }}">
    <div class="col-12 mt-2 d-flex position-relative pe-1">
      <div class="position-absolute top-0 start-0">
        <?php Helper::profilePicture('', '32px', auth()->user(), 'pointer-on-hover', ''); ?>
      </div>
      <div class="CommentMarginLeftRight2"><textarea name="" id="comment_answer_text{{ $comment->uuid }}" cols="30"
    ...
    <div class="w-100 mb-2 pe-1">
      <button onclick="answerComment('{{ $comment->uuid }}','comment')"
        id="answer_comment_btn{{ $comment->uuid }}"
        style="width:109px; padding-left: 0px !important; padding-right: 0px !important"
        class="float-end btn btn-warning btn-sm mb-1 mt-1 d-inline-block" onclick="">Kommentieren</button>
    </div>
  </div>
  <div class="d-none mt-2" id="add_comment_answer_skeleton{{ $comment->uuid }}">
    <x-comment_answer_skeleton />
  </div>
  <div class="@if ($comment->answers()->count() > 0)  my-2 @endif" id="answers_to_comment{{ $comment->uuid }}">

    {{-- @if ($comment->answers()->count())
        @foreach ($comment->answers()->get() as $commentAnswer)
          <x-comment_answer :commentAnswer="$commentAnswer" />
        @endforeach
      @endif --}}
    <span class="d-none" id="comment_answer_counter{{ $comment->uuid }}">{{ $answers->count() }}</span>
    <div id="comment_answers_container{{ $comment->uuid }}">
      @foreach ($answers as $answer)
        <x-comment_answer :commentAnswer="$answer" />
      @endforeach
    </div>
    <div class="d-none" id="comment_answers_skeletons{{ $comment->uuid }}">
      <x-comment_answer_skeleton />
      <x-comment_answer_skeleton />
      <x-comment_answer_skeleton />
    </div>

    @if ($comment->answers()->count() > 2)
      <div
        onclick="loadMoreComments('{{ $comment->uuid }}', $('#comment_answer_counter'+'{{ $comment->uuid }}').html(), 'answer')"
        id="load_more_comment_answers_btn{{ $comment->uuid }}" style="font-size: 12px"
  ...
  {{-- <div class="mb-2">
      ...
      <div class="position-relative mt-2">
        <div class="position-absolute top-0 start-0 mt-1">
          <?php Helper::profilePicture('', '32px', $comment->user, 'pointer-on-hover', ''); ?>
        </div>
        <div class="CommentMarginLeftRight2 pe-1">
          <div class="py-2 pe-2 ps-2 ps-sm-3" style="background-color: #f2f2f2; border-radius: 5px">
            ...
            <div class="mb-2 pointer-on-hover HideOverflowText"
              onclick="loadUserProfileCard('{{ $comment->user->username }}')"
              style="font-size: 12px !important; color: #7F7F7F">{{ $comment->user->current_position }} @ aktueller
              Arbeitgeber</div>
            <div class="PostBodyTextFont"
              style="width: 100% !important; white-space: pre-wrap; overflow-wrap: break-word; box-sizing:border-box; vertical-align: top">
              daslkdalsk ksdla ksdjkladasjklsadkjl sadlk asdlkdsa lsdakjlasdljkas dlk</div>
          </div>
          <div class="d-flex justify-content-start mt-1 mb-2">
            ...
            <div class=" PostLikeFont ps-2 pe-3"><img class="align-self-center mb-1" onclick="" id=""
                src="{{ asset('images/Reply.svg') }}" alt=""> 2</div>
            <div class="CommentVerticalLine" style="color: white">.</div>
            <div class="ps-3 PostLikeFont underline-on-hover align-self-center" style="font-size: 14px !important">
              Antworten</div>
          </div>
          <div class="col-12 mt-2 d-flex position-relative">
            <div class="position-absolute top-0 start-0">
              <?php Helper::profilePicture('', '32px', auth()->user(), 'pointer-on-hover', ''); ?>
            </div>
            <div class="CommentMarginLeftRight2"><textarea name="" id="" cols="30" oninput="auto_grow(this, 32)"
                class="PostBorderComment d-inline-block px-3"
                style="padding-top: 7px;padding-bottom: 5px;width:100%; height: 32px !important; overflow:hidden; -webkit-appearance: none; border-radius: 30px; font-size: 12px;"
                placeholder="Schreibe einen Kommentar..."></textarea>
            </div>
          </div>
          <button id="" class="float-end btn btn-warning btn-sm mb-1 mt-1" onclick="">Kommentieren</button>
        </div>
      </div>
    </div> --}}
</div>
